...
public class FellTheMighty : SpellImplementation
{
    public FellTheMighty(SpellOutline outline, PlayerGameState playerGameState) : base(outline, playerGameState)
    {
    }
    ...
    private async Task ResolveAllBoardsAsync()
    {
        var ordered = PlayerGameState
                      ...
                      .OrderByDescending(x => x.CreatureState.Atk)
                      .ToList();
